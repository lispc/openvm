...
use crate::{
    bigint::utils::get_fp2_arithmetic_air,
...
fn get_air_and_range_checker(
    ...
    num_limbs: usize,
    is_mul_div: bool,
) -> (Fp2ArithmeticAir, Arc<VariableRangeCheckerChip>) {
    ...
    let air = get_fp2_arithmetic_air(
        prime,
        limb_bits,
        field_element_bits,
        num_limbs,
...
fn generate_xy() -> ((BigUint, BigUint), (BigUint, BigUint)) {
    let mut rng = create_seeded_rng();
    let len = 8; // in bytes -> 256 bits.
    let x = (
        BigUint::new((0..len).map(|_| rng.next_u32()).collect()),
        BigUint::new((0..len).map(|_| rng.next_u32()).collect()),
    );
...
//     let air = Fp2MultiplicationAir { arithmetic };
...
//     let Fp2ArithmeticCols { q, r, .. } = cols.clone();
...
//     let (arithmetic, range_checker) =
//         get_air_and_range_checker(prime.clone(), LIMB_BITS, NUM_LIMB, true);
//     let air = Fp2DivisionAir { arithmetic };
...
fn test_x_add_y() {
    ...
    let air = Fp2AdditionAir { arithmetic };
    let expected_r = (
        (x.0.clone() + y.0.clone()) % prime.clone(),
        (x.1.clone() + y.1.clone()) % prime.clone(),
    );
    let expected_q = (
        (x.0.clone() + y.0.clone() - expected_r.0.clone()) / prime.clone(),
        (x.1.clone() + y.1.clone() - expected_r.1.clone()) / prime.clone(),
    );
    let cols = air.generate_trace_row((x, y, range_checker.clone()));
    let Fp2ArithmeticCols { q, r, .. } = cols.clone();
    let generated_r = (
        evaluate_bigint(&r.0, LIMB_BITS),
        evaluate_bigint(&r.1, LIMB_BITS),
    );
    let generated_q = (
        evaluate_bigint(&q.0, LIMB_BITS),
        evaluate_bigint(&q.1, LIMB_BITS),
    );
    assert_eq!(generated_r, expected_r);
    assert_eq!(generated_q, expected_q);
...
//     let (arithmetic, range_checker) =
//         get_air_and_range_checker(prime.clone(), LIMB_BITS, NUM_LIMB, false);
//     let air = Fp2SubtractionAir { arithmetic };
//     let mut xp = x.clone();
//     while xp < y {
//         xp += prime.clone();
//     }
