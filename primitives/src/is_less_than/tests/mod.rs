...
#[test]
fn test_lt_chip_lower_overflows_babybear() {
    let bus_index: usize = 0;
    let max_bits: usize = 30;
    let decomp: usize = 8;
    let range_max: u32 = 1 << decomp;
    ...
    let chip = IsLessThanChip::new(bus_index, max_bits, decomp, range_checker);
    let correct_trace_row = chip
        .generate_trace(vec![((1 << 27) - 2, 1 << 30)])
        .values
        .to_vec();
    let fake_trace_row = [(1 << 27) - 2, 1 << 30, 0, 0, 0, 0, 0, 0, 0]
        .map(BabyBear::from_canonical_u32)
        .to_vec();
    let trace = [correct_trace_row, fake_trace_row].concat();
    let trace = RowMajorMatrix::new(trace.to_vec(), 9);
    for _ in 0..5 {
        chip.range_checker.add_count(0);
    }
    let range_trace = chip.range_checker.generate_trace();
