...
>(
    ...
    // let all_public_values = iter::once(segment.get_cpu_pis()).chain()
    let all_public_values = vec![segment.get_cpu_pis(), vec![], vec![], vec![], vec![]];

    // TODO[osama]: remove this
    println!("running AIR test");

    let memory_interface_air: &dyn AnyRap<BabyBearPoseidon2Config> =
        match &segment.memory_manager.interface_chip {
            MemoryInterface::Persistent(expand_interface) => &expand_interface.air,
            MemoryInterface::Volatile(audit_interface) => &audit_interface.air,
        };

    // let Some(memory_audit_air) = segment.memory_manager.interface_chip else panic!("memory interface chip is not volatile");

    let MemoryInterface::Volatile(memory_audit_air) = &segment.memory_manager.interface_chip else {
        panic!("Memory interface chip is not Volatile");
    };

    println!("num public values: {}", all_public_values.len());

    let test_result = if field_arithmetic_enabled {
        println!("field arithmetic enabled");
        run_simple_test(
            vec![
                &segment.cpu_chip.air,
                &program_air,
                memory_interface_air,
                &arithmetic_air,
        ...
        println!("field arithmetic not enabled");
        run_simple_test(
            vec![
                &segment.cpu_chip.air,
                &program_air,
                // &segment.memory_manager.interface_chip.air,
            ],
            vec![trace, program_trace, memory_interface_trace],
            all_public_values,
        )
    };
