    ...
    ExecutionSegment<NUM_WORDS, WORD_SIZE, F>
{
    ...
    pub fn new(config: VmConfig, program: Program<F>, state: VirtualMachineState<F>) -> Self {
        ...
        let cpu_chip =
            CpuChip::from_state(config.cpu_options(), state.state, config.limb_bits, decomp);
        ...
        // );
        let memory_manager =
            MemoryManager::with_volatile_memory(memory_dimensions, decomp, range_checker.clone());
